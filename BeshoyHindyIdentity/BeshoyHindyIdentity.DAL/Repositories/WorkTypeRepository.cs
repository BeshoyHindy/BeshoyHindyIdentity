...
namespace DAL.Repositories
{
    public class WorkTypeRepository
    {
        ...
        public Guid Add()
        {
            ...
            db.WorkTypes.Add(_Obj);
            db.SaveChanges();
            return _Obj.Id;
        }
        public Boolean Edit(WorkType WorkTyperam)
        {
            try
            {
                _Obj = db.WorkTypes.FirstOrDefault(usr => usr.Id == WorkTyperam.Id);
                if (_Obj != null)
                {
                    _Obj = WorkTyperam;
                    _Obj.ModifiedOn = DateTime.Now;
                    _Obj.ModifiedBy = WorkTyperam.ModifiedBy;
                    db.SaveChanges();
                    return true;
                }
                return false;
        ...
        public Boolean Delete(Guid ID, Guid ModifiedBy)
        {
            try
            {
                _Obj = db.WorkTypes.FirstOrDefault(use => use.Id == ID);
                if (_Obj != null)
                {
        ...
        public DataTable LoadByActiveState(WorkType WorkTyperam, String SortField, String SortType)
        {
            try
            {
                if (WorkTyperam.Id != null)
                {
                    var Query = (from usr in db.WorkTypes
                                 where usr.Active == WorkTyperam.Active && usr.Id == WorkTyperam.Id
                                 select usr);
                    return Query.ToDataTable(SortField, SortType);

                }
                else if (WorkTyperam.Id == null)
                {
                    return null;
                }
                return null;
        ...
        public DataTable LoadWorksTypes()
        {
            var Query = (from wrktype in db.WorkTypes
                         where wrktype.Active == true
                         select wrktype);
            return Query.ToDataTable("RecOrder", "ASC");
        }

        public WorkType LoadByWorkType(WorkType WorkTyperam)
        {
            if (WorkTyperam != null)
            {
                _Obj = db.WorkTypes.FirstOrDefault(usr => usr.Id == WorkTyperam.Id);
        ...
        public WorkType LoadByWorkTypeId(String WorkTypeIdPram)
        {
            if (WorkTypeIdPram != null)
            {
                _Obj = db.WorkTypes.FirstOrDefault(usr => usr.Id == new Guid(WorkTypeIdPram));
                return _Obj;
            }
            return null;
        }
        ...
        {
            return (from Obj in db.WorkTypes
                    where Obj.Active == ActiveStatePram
                    select Obj.Id).Count();
        }
        public WorkType LoadByOrder(Int32 Order, Boolean ActiveStatePram)
        {
            return db.WorkTypes.FirstOrDefault(o => o.RecOrder == Order && o.Active == ActiveStatePram);
        }
        public String ReOrder(String WorkTypeId, Boolean Incre, String ModifiedBy, Boolean ActiveStatePram)
        {
            // try
            {
                WorkType Obj = db.WorkTypes.FirstOrDefault(o => o.Id == new Guid(WorkTypeId));
                if (Obj != null)
                {
                    ...
                    WorkType obj2 = LoadByOrder(NewOrder, ActiveStatePram);
                    if (obj2 != null)
                    {
        ...
        public Boolean ShiftRecords(Int32 currentOrder, Boolean ActiveStatePram)// delete & restore
        {
            try
            {
                ...
                WorkType[] ObjArr = new WorkType[RowsCount - currentOrder];
        ...
        {
            //Int32 RowsCount = LoadItemsCount(ActiveStatePram);
            //Int32 CurrentRec = (Int32)LoadByWorkTypeId(IdPram).RecOrder;
        ...
        public Boolean MoveUp(String IdPram, Boolean ActiveStatePram)
        {
            ...
            {
                WorkType CurrentRow = LoadByWorkTypeId(IdPram);
                WorkType UpperRow = LoadByOrder((Int32)CurrentRow.RecOrder - 1, ActiveStatePram);
        ...
        public Boolean MoveDown(String IdPram, Boolean ActiveStatePram)
        {
            ...
            {
                WorkType CurrentRow = LoadByWorkTypeId(IdPram);
                WorkType BottomRow = LoadByOrder((Int32)CurrentRow.RecOrder + 1, ActiveStatePram);
                int CurrentRec = (int)CurrentRow.RecOrder;
